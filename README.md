...
```
/zoom-out src/services/balance/
```

agent 输出该模块的"地图视角"（关键类、调用关系、不变量）。**把它的输出粘进 `CONTEXT.md`**——zoom-out 的真正用法不是临时问答，是给 CONTEXT.md 喂料。
...
终点是一份 `CONTEXT.md`，agent 看一眼就知道哪个概念对应哪个文件。
...
| `/zoom-out` 喂料给 CONTEXT.md | `/zoom-out <path>` 后**把它输出粘进 `CONTEXT.md`** | 给陌生大模块建地图；下次别人不再重新摸索 |
